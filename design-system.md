...
> Applied when `<html class="dark">` is set. Neutral tokens are CSS custom properties that auto-flip. Accent and semantic colors are unchanged.

### Dark Neutrals

| Token | Dark Value | CSS Variable |
|-------|-----------|--------------|
| `background` | `#0F172A` (Slate-900) | `--color-background` |
| `surface` | `#1E293B` (Slate-800) | `--color-surface` |
| `border` | `#334155` (Slate-700) | `--color-border` |
| `border-strong` | `#475569` (Slate-600) | `--color-border-strong` |
| `text-primary` | `#F8FAFC` (Slate-50) | `--color-text-primary` |
| `text-secondary` | `#94A3B8` (Slate-400) | `--color-text-secondary` |
| `text-disabled` | `#64748B` (Slate-500) | `--color-text-disabled` |
| `accent-light` | `#1E1B4B` (Indigo-950) | `--color-accent-light` |
| `accent-light-border` | `#3730A3` (Indigo-700) | `--color-accent-light-border` |
| `hover-surface` | `#1E293B` (Slate-800) | `--color-hover-surface` |
| `logs-alt-row` | `#0F172A` (Slate-900) | `--color-logs-alt-row` |

### New Tokens Added

| Token | Light | Dark | Purpose |
|-------|-------|------|---------|
| `background` | `#FFFFFF` | `#0F172A` | Page background (replaces bare `bg-white` on surfaces) |
| `hover-surface` | `#F3F4F6` | `#1E293B` | Hover state for nav items, buttons, tool blocks |
| `logs-alt-row` | `#FAFAFA` | `#0F172A` | Alternating row background in log viewer |

### Implementation Notes
- Toggle: Moon icon in light mode → click to go dark. Sun icon in dark mode → click to go light.
- Persisted in `localStorage` key `'theme'`. Falls back to `prefers-color-scheme`.
- Tremor chart deep theming deferred to follow-up change `"tremor-dark-charts"`.
